...
positions から SFEN / 評価値 / 最善手 / 読み筋 / 候補手を取得
    ↓
openings(source='self') から同一 SFEN の自分専用定跡候補を取得
    ↓
解説材料を JSON で正規化
    ↓
LLM に渡す日本語プロンプトを生成
...
初期実装では外部 LLM を直接呼び出さず、`GET /api/positions/{id}/explanation-prompt` でプロンプトと入力材料を返す。実際の生成処理、保存、ビューア表示は後続 PR で追加する。
...
| `POST` | `/api/games/import-directory` | サーバー上のフォルダから KIF を一括取り込みする（`async=true` でジョブ起動） |
| `GET` | `/api/games/import-directory/jobs/{id}` | 一括取り込みジョブの進捗を返す |
| `POST` | `/api/games/import-directory/jobs/{id}/cancel` | 一括取り込みジョブのキャンセルを要求する |
...
| `GET` | `/api/openings?sfen={sfen}&source=self` | 保存済み定跡 DB から特定局面の候補手を返す |
| `POST` | `/api/openings/rebuild` | 保存済み局面から定跡 DB を再生成する |
